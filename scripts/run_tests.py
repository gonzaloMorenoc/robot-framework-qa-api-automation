...
class WordMateTestRunner:
    ...
    def build_robot_command(self, args, config):
        ...
        cmd.extend([
            "--variable", f"ENVIRONMENT:{args.environment}",
            "--variable", f"BASE_URL:{config['environment']['base_url']}",
            "--variable", f"API_BASE_URL:{config['environment']['api_base_url']}",
            "--variable", f"BROWSER:{config['web']['browser']}",
            "--variable", f"HEADLESS:{config['web']['headless']}"
        ])
...
Examples:
  ...
  %(prog)s --list-tests
    ...
    parser.add_argument(
        "--suite",
        help="Test suite to run (api, ui, integration, smoke)"
    )
    ...
    parser.add_argument(
        "--include-tags",
        nargs="+",
        help="Include tests with specified tags"
    )
