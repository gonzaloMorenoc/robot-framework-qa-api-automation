...
class WordMateReportGenerator:
    ...
    def _extract_suite_info(self, root: ET.Element, xml_file: Path) -> Dict[str, Any]:
        ...
        if status_element is not None:
            ...
            if elapsed:
                suite_info["execution_time"] = int(elapsed) / 1000.0  # Convert to seconds
    ...
    def _extract_test_info(self, test_element: ET.Element) -> Dict[str, Any]:
        ...
        for kw_element in test_element.findall('.//kw'):
            ...
            kw_status_text = kw_status.get('status') if kw_status is not None else 'UNKNOWN'
            test_info["keywords"].append({
                "name": kw_name,
                "status": kw_status_text
            })
        ...
        if suite_element is not None:
            ...
            if status_element is not None:
                ...
                if elapsed:
                    ...
                    self.report_data["summary"]["execution_time_seconds"] += execution_time
    ...
    def _generate_execution_time_chart(self, charts_dir: Path) -> None:
        ...
        try:
            ...
            ax.set_xticklabels([f[:20] + '...' if len(f) > 20 else f for f in files], 
    ...
    def _generate_test_suite_chart(self, charts_dir: Path) -> None:
        ...
        try:
            ...
            suite_names = [suite["name"][:15] + '...' if len(suite["name"]) > 15 
                          else suite["name"] for suite in suites]
            ...
            bars1 = ax.bar([i - width/2 for i in x], passed_counts, width, 
                          label='Passed', color='#28a745', alpha=0.8)
            bars2 = ax.bar([i + width/2 for i in x], failed_counts, width,
                          label='Failed', color='#dc3545', alpha=0.8)
    ...
    def generate_html_report(self, output_dir: Path, environment: str = None, 
                           report_type: str = "standard") -> Path:
...
"""
    ...
    def generate_complete_report(self, input_dir: Path, output_dir: Path, 
                               environment: str = None, report_type: str = "standard",
                               open_report: bool = False) -> Dict[str, Path]:
        """Generate complete test report with all components"""
        print(f"Generating {report_type} report for {environment or 'unknown'} environment...")
