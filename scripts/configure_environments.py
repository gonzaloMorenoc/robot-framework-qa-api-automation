...
class EnvironmentConfigurator:
    ...
    def validate_environment_config(self, env_name: str) -> Dict[str, Any]:
        ...
        required_sections = ["environment", "web", "database", "authentication", "test_data"]
        ...
        if "your_" in config_str.lower() or "example" in config_str.lower():
            validation_result["warnings"].append("Configuration contains placeholder values")
    ...
    def run_full_validation(self, env_name: str = None) -> bool:
        ...
        environments_to_check = [env_name] if env_name else list(self.environments.keys())
        all_healthy = True
        ...
        for env in environments_to_check:
            ...
            if "error" in connectivity_results:
                print(f"❌ Connectivity test failed for {env}: {connectivity_results['error']}")
                all_healthy = False
                continue
            ...
            base_accessible = connectivity_results["base_url_check"].get("accessible", False)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Configure and validate WordMate environments")
