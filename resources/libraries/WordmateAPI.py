...
class WordmateAPI:
    ...
    def make_api_request(self, method: str, endpoint: str, data: Dict = None, 
        ...
        try:
            ...
            if response.status_code != expected_status:
                logger.warn(f"Expected status {expected_status}, got {response.status_code}")
    ...
    def register_user(self, username: str, password: str, first_name: str, 
        ...
        return self.make_api_request("POST", "?endpoint=register", registration_data, expected_status=201)
    ...
    @keyword
    def get_vocabulary_list(self, page: int = 1, limit: int = 50, 
    ...
    @keyword
    def create_custom_vocabulary(self, word: str, definition: str, 
                                pronunciation: str = None) -> Dict:
        ...
        return self.make_api_request("POST", "?endpoint=customVocabulary", data, expected_status=201)
    
    @keyword
    def create_folder(self, name: str, color: str = "#007bff", 
                     icon: str = "folder") -> Dict:
        ...
        return self.make_api_request("POST", "?endpoint=folders", data, expected_status=201)
    ...
    def generate_oauth_url(self, provider: str, redirect_url: str) -> str:
        ...
        if provider.lower() == "google":
            client_id = "190232784063-2o4nif2evn9gsnmpou8iieck52dbvv5p.apps.googleusercontent.com"
            scope = "email profile"
    ...
    @keyword
    def verify_api_response_schema(self, response_data: Dict, 
            ...
            if response["status_code"] == 200:
                for item in response["data"].get("items", []):
                    if item.get("word", "").startswith("test_"):
                        self.make_api_request("DELETE", f"?endpoint=customVocabulary&id={item['id']}")
            ...
            if response["status_code"] == 200:
                for folder in response["data"].get("folders", []):
                    if folder.get("name", "").startswith("test_"):
                        self.make_api_request("DELETE", f"?endpoint=folders&id={folder['id']}")
    ...
    @keyword
    def measure_api_performance(self, method: str, endpoint: str, 
                               data: Dict = None, iterations: int = 1) -> Dict:
